...
#no command line args given, auto select all dat files in current directory
    ...
    wantedFiles = [ROOT+f for f in listdir(ROOT) if isfile(join(ROOT, f)) and f.endswith(".dat") and "combined" not in f and "ActivityLog" not in f]

skipLines = 0
with open(ROOT+"Obs_combined.dat","w") as combinedFile:
